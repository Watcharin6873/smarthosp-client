...
import { Button, Checkbox, Divider, Empty, Form, Image, Select, Switch, Input, Modal } from 'antd'
...
const FormApproveService_Zone = () => {
  ...
  const [formUnApprove] = Form.useForm()
  ...
  const category1 = evaluateByZone.filter(f => f.category_questId === 3)
  ...
  const searchQuests = listQuests.filter(f => f.category_questId === 3)
                                      ...
                                      item2.check.split(",").map((ch) =>
                                        dataSubQuestLists.map((sb) =>
                                          sb.sub_questId === item2.sub_questId && sb.choice === ch
                                            ?
                                            <div className='flex items-baseline gap-2 mt-3 ml-7 p-2'>
                                              <Checkbox checked />
                                              <p
                                                className={
...
export default FormApproveService_Zone